...
def BM25_score_and_idf(index_read, query, str_index, weight):
    ...
    for term in query:
        if index_read.df.get(term) is None:
            break
        else:
            if str_index == 'body':
...
def search_second(dict_search, pagerank_scores):
    ...
    for doc_id, bm25_score in dict_search.items():
        ...
        combined_scores[doc_id] = 0.6 * bm25_score + 0.4 * math.log(pagerank_score + 1)
...
def get_top_n(dirug):
    # Use heapq.nlargest to directly get the top 100 items
